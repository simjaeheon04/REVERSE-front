...
export default function NoticeManagePage() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setSaveMessage(form.noticeId ? "공지사항이 수정되었습니다." : "공지사항이 등록되었습니다.");
    ...
    <S.Page>
      <S.Shell>
        <S.Header>
          ...
          <S.Description>
            공지사항 목록, 상세 조회, 이미지 업로드, 등록, 수정, 삭제를 한 곳에서 관리합니다.
          </S.Description>
        </S.Header>
        ...
        <S.Grid>
          <S.Card>
            ...
            <S.CardText>
              먼저 이미지를 업로드한 뒤, 업로드된 URL을 공지사항 payload에 포함합니다.
            </S.CardText>
            ...
            {uploadMessage ? (
              <S.StatusText $error={!uploadedImageUrl}>{uploadMessage}</S.StatusText>
            ) : null}

            <S.PreviewPanel>
              {uploadedImageUrl ? (
                <S.PreviewImage src={uploadedImageUrl} alt="공지 이미지 미리보기" />
              ) : (
                <S.EmptyPreview>업로드한 이미지 미리보기가 여기에 표시됩니다.</S.EmptyPreview>
              )}

              <S.CodeBlock>{JSON.stringify(form.imageUrls, null, 2)}</S.CodeBlock>
            </S.PreviewPanel>
          </S.Card>
          ...
          <S.Card>
            ...
            <S.CardText>
              API.md 기준 payload에 맞춰 카테고리, 외부 공개 여부, 이미지 URL 목록까지 함께 저장합니다.
            </S.CardText>
            ...
            <S.Form onSubmit={handleSubmit}>
              ...
              <S.InlineFields>
                <S.Field>
                  ...
                  <S.Select value={String(form.isPinned)} onChange={handleBooleanChange("isPinned")}>
              ...
              <S.Field>
                ...
                <S.Input value={form.noticeId ?? ""} readOnly placeholder="수정 모드에서 자동 입력됩니다." />
              </S.Field>

              <S.CardTitle as="h3">첨부 이미지</S.CardTitle>
              <S.ButtonRow>
                {form.imageUrls.map((imageUrl) => (
                  <S.SecondaryButton key={imageUrl} type="button" onClick={() => handleRemoveImage(imageUrl)}>
                    이미지 제거
                  </S.SecondaryButton>
                ))}
              </S.ButtonRow>
              ...
              <S.ButtonRow>
                <S.PrimaryButton type="submit">
                  {isSaving ? "저장 중..." : isEditMode ? "공지 수정" : "공지 등록"}
                </S.PrimaryButton>
            ...
            {saveMessage ? <S.StatusText $error={!saveResult}>{saveMessage}</S.StatusText> : null}

            <S.PreviewPanel>
              <S.CardTitle as="h3">요청 미리보기</S.CardTitle>
              <S.CodeBlock>{JSON.stringify(form, null, 2)}</S.CodeBlock>
              <S.CardTitle as="h3">응답</S.CardTitle>
              <S.CodeBlock>{saveResult ? JSON.stringify(saveResult, null, 2) : "아직 응답이 없습니다."}</S.CodeBlock>
            </S.PreviewPanel>
          </S.Card>
          ...
          <S.Card>
            ...
            <S.CardText>
              <code>GET /api/notices</code>의 페이지 목록을 확인하고 수정할 공지를 선택합니다.
            </S.CardText>

            <S.ButtonRow>
              <S.SecondaryButton type="button" onClick={() => void loadNotices(currentPage)}>
              ...
              <S.SecondaryButton
                type="button"
                onClick={() => void loadNotices(Math.min(currentPage + 1, Math.max(totalPages - 1, 0)))}
            ...
            {listMessage ? <S.StatusText $error={!notices.length}>{listMessage}</S.StatusText> : null}

            <S.CodeBlock>{JSON.stringify({ currentPage, totalPages, notices }, null, 2)}</S.CodeBlock>

            <S.CardTitle as="h3">상세 조회</S.CardTitle>
            <S.ButtonRow>
              {notices.map((notice) => (
                <S.SecondaryButton key={notice.id} type="button" onClick={() => void handleLoadDetail(notice.id)}>
